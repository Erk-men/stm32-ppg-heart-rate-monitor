...
- Circular buffer `s_ma_buf[32]` with running sum `s_ma_sum` (uint32_t, overflow-safe for 32 x 4095 = 131040)
...
- 100-entry `sine_table[100]` under `#ifdef SYNTHETIC_TEST`:
  ...
  - Loop period: 100 x 10ms = 1.0s = 60 BPM when the table loops continuously (D-08)
...
- SIG-02: adaptive threshold (last_peak x 0.6)
...
Files created: Inc/algorithm.h, Src/algorithm.c, .planning/phases/03-algorithm-modules-on-synthetic-data/03-01-SUMMARY.md
Files modified: Src/main.c
Commits: 44cc83a (Task 1), b4d2ae7 (Task 2)
