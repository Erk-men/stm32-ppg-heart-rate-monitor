...
- [ ] **Phase 2: ADC + Timer Hardware Trigger** - TIM2 hardware-triggers ADC1 at exactly 100Hz; raw values stream over serial
...
**Success Criteria** (what must be TRUE):
  ...
  2. PA0 tied to 3.3V (VDDA) reads ≈ 4095; PA0 tied to GND reads ≈ 0 — confirming 12-bit range and ADCPRE=/6 (ADCCLK=12MHz)
  3. EXTSEL bits in ADC1->CR2 match RM0008 Table 68 value for TIM2_TRGO; ADC fires only when TIM2 update event fires
  4. ADC calibration sequence (RSTCAL then CAL) completes at startup with no hang
**Plans**: TBD
...
**Success Criteria** (what must be TRUE):
  ...
  2. Register configuration table documents every peripheral register written (RCC, GPIO, TIM2, ADC1, USART2, SysTick) with the hex value and plain-English rationale
